...
export default function SpacePage() {
  ...
  useEffect(() => {
    ...
    }).catch(() => setError("Failed to join space"))
  ...
  if (loading) return (
    <div className="fixed inset-0 z-50 bg-[#06091a] flex items-center justify-center">
      <div className="flex flex-col items-center gap-4">
        ...
        <p className="text-white/40 text-sm">Joining space…</p>
      </div>
    </div>
  );
  ...
  if (error || !space || !token) return (
    <div className="fixed inset-0 z-50 bg-[#06091a] flex flex-col items-center justify-center gap-5">
      <p className="text-white/50 text-lg">{error ?? "Space unavailable"}</p>
      <button onClick={() => router.push("/spaces")} className="px-6 py-2.5 rounded-full bg-violet-600 text-white font-bold text-sm hover:bg-violet-500 transition-all">Back to Spaces</button>
    </div>
  );
